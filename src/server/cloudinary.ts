...
const replacePlaceholder = (cursor: Cursor, placeholder: string, replaceWith: string) => {
	cursor.setValue(cursor.value.replace(placeholder, replaceWith))
}

export const uploadImageCommandHandler = async (textareaEl: HTMLTextAreaElement, files: File[]) => {
	...
	const processFiles = async () => {
		for (const file of files) {
			...
			try {
				...
				const width = uploadedImage.dpi >= 144 ? Math.round(uploadedImage.width / 2) : uploadedImage.width
