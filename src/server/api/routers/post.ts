...
export const postRouter = createTRPCRouter({
		...
		.query(async ({ ctx, input }) => {
			...
			return {
				posts: posts.map(post => ({
					...post,
					isLikedByCurrentUser: Boolean(post.likedBy.find(item => item.user.id === ctx.session.user.id)),
					likedBy: post.likedBy.map(like => (like.user.id === ctx.session.user.id ? 'You' : like.user.name)),
				})),
				postCount,
			}
		}),
	detail: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			if (!post || (post.hidden && ctx?.session?.user?.role !== 'ADMIN')) {
				throw new TRPCError({
					code: 'NOT_FOUND',
	...
	unlike: protectedProcedure.input(z.object({ id: z.number() })).mutation(async ({ ctx, input }) => {
		const { id } = input
	...
	hide: protectedProcedure.input(z.number()).mutation(async ({ ctx, input: id }) => {
		if (!ctx.isUserAdmin) {
			throw new TRPCError({ code: 'FORBIDDEN' })
		}
	...
	unhide: protectedProcedure.input(z.number()).mutation(async ({ ctx, input: id }) => {
		if (!ctx.isUserAdmin) {
			throw new TRPCError({ code: 'FORBIDDEN' })
		}
