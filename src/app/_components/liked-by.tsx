				...
				>
					<div className='inline-flex items-center gap-1.5'>
						{isLikedByCurrentUser ? (
							<HeartFilledIcon className='w-4 h-4 text-red-fuerza' />
						) : (
							<HeartIcon className='w-4 h-4 text-red-fuerza' />
						)}
						<span className='text-sm font-normal tabular-nums font-versos'>{likeCount}</span>
					</div>
				</Tooltip.Trigger>
				<Tooltip.Content
					side='bottom'
					...
					className={classNames(
						'max-w-[260px] px-3 py-1.5 rounded shadow-lg bg-secondary-inverse text-secondary-inverse sm:max-w-sm font-versos',
						likeCount === 0 && 'hidden',
					)}
				>
					<p className='text-sm '>
