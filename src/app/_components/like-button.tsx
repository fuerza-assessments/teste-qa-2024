...
export const ReactionButton = ({ id, likeCount, isLikedByCurrentUser }: LikeButtonProps) => {
	...
	return (
		<Button
			variant='secondary'
			className={classNames('transition-colors overflow-hidden [transform:translateZ(0)] space-x-1.5', {
				'border-red-300 !bg-red-100 dark:!bg-red-900 dark:border-red-700': isLikedByCurrentUser,
		...
		>
			<span className='relative block w-4 h-4 shrink-0'>
				{isLikedByCurrentUser && !isAnimating ? (
					...
					<>
						<HeartIcon
							className={classNames('absolute inset-0 transition-all text-red fill-transparent transform-gpu', {
