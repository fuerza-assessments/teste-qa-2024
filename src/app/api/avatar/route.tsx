...
const generateSvg = async (name: string) => {
	...
	const colors = [...Array.from({ length: 3 })].map((_, idx) => {
		...
		const firstBit = colorHash[0]
		const secondBit = colorHash[1]

		if (!firstBit || !secondBit) {
			return COLORS.red[500]
		}

		const nameDecimal = parseInt(firstBit, 16)
		const colorName = COLOR_NAMES[nameDecimal % COLOR_NAMES.length] ?? 'red'

		const shadeDecimal = parseInt(secondBit, 16)
		const colorShade = COLOR_SHADES[shadeDecimal % COLOR_SHADES.length] ?? 500
