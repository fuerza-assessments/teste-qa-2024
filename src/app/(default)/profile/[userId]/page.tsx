...
export const generateMetadata = async ({ params }: ProfilePageParams) => {
	...
	return {
		title: `${profile.name} - Forte`,
	}
}

const POSTS_PER_PAGE = 20

export default async function ProfilePage({ params, searchParams }: ProfilePageParams) {
	...
	const { posts, postCount } = await api.post.feed.query({
		take: POSTS_PER_PAGE,
		skip: currentPageNumber === 1 ? undefined : POSTS_PER_PAGE * (currentPageNumber - 1),
	...
	return (
		<>
			<div className='relative flex items-center gap-4 py-8 overflow-hidden'>
				<div className='flex items-center gap-8'>
					...
					<div className='flex-1'>
						<h1 className='bg-primary text-2xl font-semibold tracking-tight md:text-3xl'>{profile.name}</h1>
						{profile.title && <p className='text-lg tracking-tight text-secondary'>{profile.title}</p>}
					</div>
				</div>
			...
			<Pagination itemCount={postCount} itemsPerPage={POSTS_PER_PAGE} currentPageNumber={currentPageNumber} />
		</>
	)
}
