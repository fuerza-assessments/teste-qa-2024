	...
	return (
		<>
			<AlertDialogContent>
				...
				<div className='grid grid-flow-col gap-6 mt-6'>
					<div className='text-center'>
						...
						/>
						<p className='mt-2 text-xs text-secondary'>JPEG, PNG, GIF / 5MB max</p>
					</div>
					{uploadedImage && (
						<div className='text-center'>
							<Button
							...
							<p className='mt-2 text-xs text-secondary'>And use default avatar</p>
						</div>
					)}
				</div>
			</AlertDialogContent>
			<AlertDialogActions>
				<AlertDialogAction>
					<Button
						isLoading={updateUserAvatarMutation.isLoading || uploadImageMutation.isLoading}
...
export const UpdateAvatarAction = ({ name, image }: { name: string; image: string | null }) => {
	const { handleDialog } = useDialogStore()
	return (
		<button
			type='button'
			className='relative inline-flex group'
			onClick={() => handleDialog({ content: <UpdateAvatar user={{ name, image }} /> })}
		>
