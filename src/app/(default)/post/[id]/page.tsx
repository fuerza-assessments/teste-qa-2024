...
export const generateMetadata = async ({ params }: PostPageParams) => {
	...
	if (!post) {
		return {
			title: 'Post Not found - Forte',
		}
	}

	return {
		title: `${post.title} - Forte`,
	}
}
...
export default async function PostPage({ params }: PostPageParams) {
	...
	return (
		<article className='divide-y divide-primary'>
			<div className='pb-12'>
				{post.hidden && (
					<Banner className='mb-6'>This post has been hidden and is only visible to administrators.</Banner>
				)}

				<div className='flex items-center justify-between gap-4'>
					<h1 className='text-4xl font-versos font-semibold tracking-tighter md:text-4xl'>{post.title}</h1>
